...
function TelaLogin({ onLogin }) {
  ...
  function digitar(n) {
    ...
    if (novo.length === 4) {
      ...
      setTimeout(() => {
        ...
        else { setErro(true); setPin(""); setLoading(false); }
      }, 400);
    }
  }
  ...
  return (
    <div style={{
      ...
      <div style={{ height: 20, marginBottom: 20, fontSize: 13, color: erro ? "#FFB3B3" : "rgba(255,255,255,0.4)", fontWeight: erro ? 600 : 400 }}>
        {erro ? "❌ PIN incorreto. Tente novamente." : loading ? "Verificando..." : pin.length > 0 ? "•".repeat(pin.length) + "○".repeat(4-pin.length) : ""}
      </div>

      {/* Keypad */}
      <div style={{ display: "grid", gridTemplateColumns: "repeat(3,1fr)", gap: 12, width: 260 }}>
        {[1,2,3,4,5,6,7,8,9].map(n => (
          <button key={n} onClick={() => digitar(String(n))} style={{
            height: 64, borderRadius: T.radius,
            border: "1px solid rgba(255,255,255,0.12)",
            background: "rgba(255,255,255,0.08)",
            fontSize: 24, fontWeight: 600, color: T.white,
